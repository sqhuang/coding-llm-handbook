...
def _resolve_paths() -> tuple[Path, Path]:
    """Locate tracker.json — prefer cwd, fall back to bundle root.

    Lets tests + alternate experiments override by chdir, without needing
    a CLI flag in every invocation.
    """
    bundle_root = Path(__file__).resolve().parent.parent
    cwd_tracker = Path.cwd() / "tracker.json"
    if cwd_tracker.exists():
        return cwd_tracker, Path.cwd() / "tracker_view.md"
    return bundle_root / "tracker.json", bundle_root / "tracker_view.md"


TRACKER, VIEW_MD = _resolve_paths()
ROOT = TRACKER.parent
