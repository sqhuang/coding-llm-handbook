...
running tools inside a Docker sandbox.
"""
import json, os, re, uuid, time, textwrap
...
class ToolExecutor:
    def __init__(self, sbx: Sandbox): self.sbx = sbx
    ...
    def edit_file(self, path, old_str, new_str):
        ...
        if n > 1:  return f"ERROR: old_str matches {n} places; make it unique"
    ...
    def finish(self, summary): return f"FINISHED: {summary}"

    def dispatch(self, name, args):
        fn = getattr(self, name, None)
        if not fn: return f"ERROR: no tool {name}"
        try: return fn(**args)
        except Exception as e: return f"ERROR in {name}: {e}"
...
def run_agent(task: str, sbx: Sandbox, max_steps=25):
    ...
    for step in range(max_steps):
        ...
        for tc in m.tool_calls:
            ...
            msgs.append({"role":"tool","tool_call_id": tc.id,"name": name,"content": result})
